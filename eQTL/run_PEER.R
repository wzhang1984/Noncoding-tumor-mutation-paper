...
expr = read.delim('../pat2exp_log2.txt', header=T, row.names=1)
...
colnames(factors)=c(rownames(covs),'mean',paste("h", seq(1:20), sep = ""))

residuals = PEER_getResiduals(model)
rownames(residuals)=rownames(expr_zscore)
colnames(residuals)=colnames(expr_zscore)

write.table(t(factors),'factors_20.txt',sep='\t',quote=F,col.names=F)
write.table(t(residuals),'residuals_20.txt',sep='\t',quote=F,col.names=F)
write.table(t(expr_zscore),'expr_zscore.txt',sep='\t',quote=F,col.names=F)

alpha = PEER_getAlpha(model)
rownames(alpha)=colnames(factors)
write.table(alpha,'precision_20.txt',quote=F,sep='\t',row.names=T,col.names=F)
alpha=alpha[25:length(alpha)]
